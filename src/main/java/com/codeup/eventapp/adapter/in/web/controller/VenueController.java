...
import com.codeup.eventapp.adapter.in.web.mapper.VenueDtoMapper;
...
/**
 * REST Controller for Venue management (Input Adapter)
...
public class VenueController {
    ...
    public ResponseEntity<AppResponse<VenueResponse>> crear(
        ...
        Venue venue = mapper.toDomain(req);
        Venue created = venueUseCase.createVenue(venue);
        VenueResponse res = mapper.toResponse(created);
    ...
    public ResponseEntity<AppResponse<List<VenueResponse>>> list(
        ...
        Page<Venue> page = venueUseCase.listVenues(pageable);
        List<VenueResponse> responses = page.map(mapper::toResponse).getContent();
    ...
    public ResponseEntity<AppResponse<List<VenueResponse>>> listDeleted(
        @Parameter(description = "Pagination parameters") Pageable pageable) {
        Page<Venue> page = venueUseCase.listDeletedVenues(pageable);
        List<VenueResponse> responses = page.map(mapper::toResponse).getContent();
        return ResponseEntity.ok(AppResponse.withPage(responses, page));
    }    
    ...
    public ResponseEntity<AppResponse<VenueResponse>> get(
        ...
        Venue venue = venueUseCase.getVenue(id);
        return ResponseEntity.ok(AppResponse.success(mapper.toResponse(venue)));
    }
    ...
    public ResponseEntity<AppResponse<VenueResponse>> update(
        ...
        @Valid @RequestBody VenueRequest req) {
        Venue venue = mapper.toDomain(req);
        Venue updated = venueUseCase.updateVenue(id, venue);
